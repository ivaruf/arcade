...
for tag,d in zip(['Welcome','Race','Water','Mine','Calm','Dam','Adventure','Neonfox'],DECKS):
    ...
    tube(tag+' inset landing light',pts,.018,[cyan,gold,cyan,gold,pink,cyan,gold,cyan][DECKS.index(d)],True)
...
exec(compile((OUT / 'source' / 'neonfox_island.py').read_text(), str(OUT / 'source' / 'neonfox_island.py'), 'exec'), globals())
