...
# Everything below is authored about the island's own centre line, so every
# x is an OFFSET and has to be turned into a world coordinate. That used to
# be omitted entirely, which was invisible for exactly as long as the
# steamworks happened to sit at x=0 — the day it moved west, its gears, its
# boiler and its compass inlay stayed behind on the welcome cloud.
def X(dx=0.0): return RX+dx
...
for i in range(19):
    box('Steamworks deck plank',(X(),RY-.065,RZ-3.6+i*.4),(8.2,.1,.375),wood if i%3 else woodlight,.018)
for x in [-4.12,4.12]:box('Steamworks deck edging',(X(x),RY-.015,RZ),(.14,.13,7.65),iron,.025)
for z in [RZ-3.8,RZ+3.8]:box('Steamworks deck edging',(X(),RY-.015,z),(8.38,.13,.14),iron,.025)
for x in [-3.95,3.95]:
    for i in range(13):cylinder('Deck rivet',(X(x),RY+.016,RZ-3.5+i*.58),.034,.025,bronze)
ring('Workshop compass inlay',(X(),RY-.004,RZ+1.15),1.42,.025,bronze)
for i in range(12):
    a=i*math.tau/12
...
for yy in [.30,.62,1.55,1.83]:ring('Boiler band',(X(),RY+yy,BZ),.59,.052,bronze)
sphere('Boiler crown',(X(),RY+1.78,BZ),.58,copper,scale=(1,.4,1),segs=24)
cylinder('Chimney collar',(X(),RY+2.03,BZ),.25,.18,iron)
cylinder('Chimney',(X(),RY+2.68,BZ),.17,1.25,copper)
for yy in [2.12,2.7,3.29]:ring('Chimney band',(X(),RY+yy,BZ),.18,.03,bronze)
cylinder('Chimney cap',(X(),RY+3.36,BZ),.28,.09,iron)
# Gauge faces point into the approach, toward game +Z.
cylinder('Pressure gauge housing',(X(.20),RY+1.45,BZ+.57),.20,.10,bronze,axis='z')
cylinder('Pressure gauge dial',(X(.20),RY+1.45,BZ+.63),.165,.015,white,axis='z')
for i in range(9):
    a=math.radians(30+i*30)
    beam('Gauge graduation',(X(.20+.13*math.cos(a)),RY+1.45+.13*math.sin(a),BZ+.646),(X(.20+.15*math.cos(a)),RY+1.45+.15*math.sin(a),BZ+.646),.012,black)
beam('Gauge needle',(X(.20),RY+1.45,BZ+.655),(X(.10),RY+1.53,BZ+.655),.014,pink)
ring('Furnace door',(X(),RY+.68,BZ+.57),.28,.042,iron,axis='z')
cylinder('Furnace glow',(X(),RY+.68,BZ+.575),.24,.02,amber,axis='z')
for dx in [-.15,-.075,0,.075,.15]:box('Furnace grille',(X(dx),RY+.68,BZ+.6),(.026,.39,.04),iron,.005)
for i in range(16):
    a=i*math.tau/16
